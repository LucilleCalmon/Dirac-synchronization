...
tau1 = 1/sqrt(N-1);%edges frequency precision scaled appropriately
...
omegaedge = omegaedge-diag(diag(omegaedge)); %Gaussian links frequencies
what=sum(omegaedge,2)-(sum(omegaedge,1))';
what = what/(N-1); %Appropriately scaled projected links frequencies onto nodes
...
dsigma=0.03;
func = @ddt_II_fc; %dynamical equations to be loaded from ddt_II_fc.m
z = 5; %value of z
...
for sigma = 0:dsigma:sigma_max %sweep sigma up
    ...
    for t = 0:dt:Tmax  %time integration
        ...
        %Runge-Kutta 4
        [k1{1},k1{2}] = func(theta, psi, sigma, w,what,z);
        [k2{1},k2{2}] = func(theta+0.5*dt*k1{1}, psi+0.5*dt*k1{2},sigma,w,what,z);
        [k3{1},k3{2}] = func(theta+0.5*dt*k2{1}, psi+0.5*dt*k2{2},sigma,w,what,z);
        [k4{1},k4{2}] = func(theta+dt*k3{1}, psi+dt*k3{2},sigma,w,what,z);
